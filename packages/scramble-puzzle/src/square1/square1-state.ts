...
const createSquareOneState = (sliceSolved: boolean, pieces: readonly number[]): SquareOneState => {
  if (pieces.length !== PIECE_COUNT) {
    throw new RangeError(`square1 state must contain 24 piece positions: ${pieces.length}`);
  }

  for (const piece of pieces) {
    if (!Number.isSafeInteger(piece) || piece < 0 || piece > 15) {
      throw new RangeError('square1 pieces must be integer piece indexes from 0 to 15');
    }
  }
...
const rotateHalf = (pieces: SquareOnePiece[], start: number, turn: number): void => {
...
export const areSquareOneStatesEqual = (a: SquareOneState, b: SquareOneState): boolean =>
...
export const getSquareOneSuccessors = (state: SquareOneState): readonly SquareOneSuccessor[] => {
...
export const getSquareOneScrambleSuccessors = (
  ...
  getSquareOneSuccessors(state).filter((successor) => canSquareOneSlash(successor.state));
