// @ts-nocheck
/* eslint-disable func-style, no-unused-vars, unicorn/no-new-array */
// Mechanical ESM port of cubing/min2phase.js, GPL-3.0 licensed.
  ...
  this.setMPerm = function setMPerm(idx) {
    ...
    for (var i = 10; i >= 8; i--) {
      ...
      for (var j = i + 1; j < 12; j++) {
        if (this.ep[j] >= this.ep[i]) this.ep[j]++;
      }
    }
  };
...
function Solve(c) {
  ...
  for (var i = 0; i < 6; i++) {
    ...
    prun[i] = Math.max(
      Math.max(
        UDSliceTwistPrun[twist[i] * 495 + UDSliceConj[slice[i]][tsym[i]]],
  ...
  for (length1 = 0; length1 < sol; length1++) {
    ...
    for (urfidx = 0; urfidx < 6; urfidx++) {
      ...
      if (
        prun[urfidx] <= length1 &&
        phase1(twist[urfidx], tsym[urfidx], flip[urfidx], fsym[urfidx], slice[urfidx], length1, 18)
      ) {
        if (solution == null) {
          return 'Error 8';
        } else {
...
function phase1(twist, tsym, flip, fsym, slice, maxl, lm) {
  ...
  for (var m = 0; m < 18; m++) {
    ...
    flipx >>>= 3;
    if (
      TwistFlipPrun[twistx * 2688 + ((flipx << 3) | Sym8MultInv[fsymx][tsymx])] >= maxl ||
...
function init2() {
  ...
  var ex =
    merge[~~(e1[length1] / 6)][~~(e2[length1] / 6)] * 4032 +
    e1[length1] * 12 +
    (e2[length1] % 6) * 2 +
    (((CParity[cornx >>> 3] >>> (cornx & 7)) & 1) ^ get4Parity(mid));
