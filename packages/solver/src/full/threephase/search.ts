...
export class Search {
  ...
  private doSearch(): void {
    ...
    if (p1SolsArr[0] === undefined) {
      throw new Error('@cubegin/solver: threephase phase 1 found no candidates');
    }
    ...
    if (arr2.length === 0) {
      throw new Error(
        `@cubegin/solver: threephase phase 2 found no candidates (phase1=${this.p1sols.length}, firstValue=${String(p1SolsArr[0]?.value)}, arr2idx=${this.arr2idx}, arr2len=${this.arr2.length})`,
      );
    }
    let length123 = 100;
    let index = 0;
    ...
    if (sol.startsWith('Error')) {
      throw new Error(`@cubegin/solver: min2phase returned ${sol} during 4x4 reduction`);
    }
  ...
  private init2(sym: number, _lastMove: number): boolean {
    ...
    if (
      ...
      throw new Error(`@cubegin/solver: invalid phase 2 coordinate ct=${s2ct} rl=${s2rl}`);
    }
