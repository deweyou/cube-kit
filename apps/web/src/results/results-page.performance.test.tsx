  ...
  const Select = {
    Root: ({
      ...
      Children.forEach(children, (child) => {
        if (!isValidElement<{ children?: ReactNode }>(child) || child.type !== Content) return;
...
describePerformance('ResultsPage performance', () => {
  ...
  it(
    'keeps the 12,000-solve list virtual and charts responsive',
    async () => {
      ...
      const renderedRowCount = table.querySelectorAll(
        '[data-component-virtual-list="true"] > *',
      ).length;
      console.info(
      ...
      console.info(
        `[results-performance] distribution duration=${formatDuration(distributionDuration)}`,
      );
