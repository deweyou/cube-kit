...
describe('useTimerGesture — H5 touch', () => {
  ...
  it('touchend before 300ms does NOT call onStart', () => {
    ...
    act(() => {
      document.dispatchEvent(new TouchEvent('touchend', { changedTouches: [makeTouch(400)], bubbles: true }))
    })
    expect(onStart).not.toHaveBeenCalled()
  })
  ...
  it('touchend in cancel zone (Y < 80) when running calls onCancel', () => {
    ...
    renderHook(() =>
      useTimerGesture(true, { onStart: vi.fn(), onStop: vi.fn(), onCancel }),
    )
    act(() => {
      document.dispatchEvent(
  ...
  it('touchend outside cancel zone (Y >= 80) when running calls onStop', () => {
    const onStop = vi.fn()
    renderHook(() =>
      useTimerGesture(true, { onStart: vi.fn(), onStop, onCancel: vi.fn() }),
    )
    act(() => {
      document.dispatchEvent(
