...
import { TimerV2Page } from './timer-v2-page';
...
vi.mock('../timer/scramble-worker-client', () => ({
...
describe('TimerV2Page', () => {
  ...
  it('shows a responsive icon-only primary navigation', () => {
    ...
    expect(timerV2Css).toMatch(/\.hero\s*\{[^}]*gap: 14px;/su);
    expect(timerV2Css).toMatch(
      /\.brandRow\s*\{[^}]*grid-template-columns: minmax\(0, 1fr\) auto;/su,
    );
    expect(timerV2Css).toMatch(/\.brandRow\s*\{[^}]*min-height: 50px;/su);
    expect(timerV2Css).toMatch(/\.root\s*\{[^}]*position: relative;/su);
    expect(timerV2Css).toMatch(/\.primaryNav\s*\{[^}]*border-radius: 999px;/su);
    expect(timerV2Css).toMatch(
    ...
    expect(timerV2Css).toMatch(/\.primaryNav\s*\{[^}]*0 8px 18px color-mix/su);
    expect(timerV2Css).not.toMatch(/\.primaryNav\s*\{[^}]*var\(--ui-color-brand-text\)/su);
    expect(timerV2Css).toMatch(/\.primaryNav\s*\{[^}]*isolation: isolate;/su);
    expect(timerV2Css).toMatch(/\.primaryNav\s*\{[^}]*overflow: hidden;/su);
    expect(timerV2Css).toMatch(/\.primaryNav\s*\{[^}]*justify-self: center;/su);
    expect(timerV2Css).toMatch(
      /\.primaryNav\s*\{[^}]*inset-block-start: calc\(18px \+ env\(safe-area-inset-top\)\);/su,
    );
    expect(timerV2Css).toMatch(/\.primaryNav\s*\{[^}]*inset-inline-start: 50%;/su);
    expect(timerV2Css).toMatch(/\.primaryNav\s*\{[^}]*position: absolute;/su);
    expect(timerV2Css).toMatch(/\.primaryNav\s*\{[^}]*transform: translateX\(-50%\);/su);
    expect(timerV2Css).toMatch(/\.primaryNav\s*\{[^}]*z-index: 5;/su);
    expect(timerV2Css).toMatch(/\.primaryNav::before\s*\{[^}]*background: linear-gradient/su);
    expect(timerV2Css).toMatch(/\.primaryNav::after\s*\{[^}]*box-shadow: inset 0 -8px 14px/su);
    expect(timerV2Css).toMatch(/\.primaryNav\s*\{[^}]*gap: 4px;/su);
    expect(timerV2Css).toMatch(/\.primaryNav\s*\{[^}]*padding: 4px 8px;/su);
    expect(timerV2Css).toMatch(
    ...
    expect(timerV2Css).toMatch(/\.navButton\s*\{[^}]*height: 42px;/su);
    expect(timerV2Css).toMatch(/\.navButton\s*\{[^}]*width: 42px;/su);
    expect(timerV2Css).toMatch(
    ...
    expect(timerV2Css).toMatch(/\.navButton::before\s*\{[^}]*inset: 2px;/su);
    expect(timerV2Css).toMatch(/\.navButton::before\s*\{[^}]*opacity: 0;/su);
    expect(timerV2Css).toMatch(
    ...
    expect(timerV2Css).toMatch(/\.navButton\[data-active='true'\]\s*\{[^}]*color:/su);
    expect(timerV2Css).toMatch(
      /\.navButton\[data-active='true'\]::before\s*\{[^}]*background: color-mix\(in srgb, var\(--ui-color-text\) 10%, transparent\);/su,
    );
    expect(timerV2Css).toMatch(/\.navButton\[data-active='true'\]::before\s*\{[^}]*opacity: 1;/su);
    expect(timerV2Css).toMatch(
    ...
    expect(timerV2Css).toMatch(/\.navButton:active::before\s*\{[^}]*opacity: 1;/su);
    expect(timerV2NavigationSource).toMatch(/TimerNavIcon/su);
    expect(timerV2NavigationSource).toMatch(/ResultsListNavIcon/su);
    expect(timerV2NavigationSource).toMatch(/FormulaStudyNavIcon/su);
    expect(timerV2NavigationSource).toMatch(/SettingsGearNavIcon/su);
    expect(timerV2NavigationSource).not.toMatch(/SettingsSlidersNavIcon/su);
  });
  ...
  it('uses a compact list selector with toolbar actions and default lists for every event', () => {
    ...
    expect(timerV2Css).toMatch(/\.listTrigger\s*\{[^}]*font-size: 0\.98rem;/su);
    expect(timerV2Css).toMatch(/\.listTrigger\s*\{[^}]*height: 32px;/su);
    expect(timerV2Css).toMatch(/\.listTrigger\s*\{[^}]*min-height: 32px;/su);
    expect(timerV2Css).toMatch(/\.listTrigger\s*:global\(svg\)\s*\{[^}]*height: 13px;/su);
    expect(timerV2Css).toMatch(/\.listToolbar\s*\{[^}]*display: flex;/su);
    expect(timerV2Css).toMatch(/\.listToolbar\s*\{[^}]*gap: 8px;/su);
    expect(timerV2Css).toMatch(/\.listToolbar\s*\{[^}]*justify-content: space-between;/su);
    expect(timerV2Css).toMatch(/\.listToolbar\s*\{[^}]*padding: 4px 6px 7px;/su);
    expect(timerV2Css).toMatch(/\.listToolbarLabel\s*\{[^}]*font-size: 0\.78rem;/su);
    expect(timerV2Css).toMatch(/\.listToolbarActions\s*\{[^}]*display: inline-flex;/su);
    expect(timerV2Css).toMatch(/\.listToolbarButton\s*\{[^}]*height: 28px;/su);
    expect(timerV2Css).toMatch(/\.listItem\s*\{[^}]*font-weight: 400;/su);
    expect(timerV2Css).toMatch(/\.listItem\[data-state='checked'\]\s*\{[^}]*font-weight: 620;/su);
  ...
  it('lays out the scramble, session summary, and scramble image around the timer', async () => {
    const { container } = render(<TimerV2Page />);
    ...
    expect(timerV2Css).toMatch(/--timer-v2-top-zone-height: clamp\(292px, 28vh, 392px\);/u);
    expect(timerV2Css).toMatch(/--timer-v2-page-inline-padding: 16px;/u);
    expect(timerV2Css).toMatch(/--timer-v2-scramble-line-height: 1\.26;/u);
    expect(timerV2Css).toMatch(/--timer-v2-scramble-toolbar-height: 36px;/u);
    expect(timerV2Css).toMatch(/--timer-v2-stage-center-offset: calc\(/u);
    expect(timerV2Css).toMatch(/--timer-v2-bottom-zone-height: clamp\(190px, 22vh, 236px\);/u);
    expect(timerV2Css).toMatch(/--timer-v2-nav-zone-height: 0px;/u);
    expect(timerV2Css).toMatch(
      /grid-template-rows:\s*var\(--timer-v2-top-zone-height\)\s*minmax\(0, 1fr\)\s*var\(--timer-v2-bottom-zone-height\);/u,
    );
    expect(timerV2Css).toMatch(/\.hero\s*\{[^}]*grid-template-rows: auto minmax\(0, 1fr\);/su);
    expect(timerV2Css).toMatch(
      /\.hero\s*\{[^}]*padding: calc\(18px \+ env\(safe-area-inset-top\)\) var\(--timer-v2-page-inline-padding\) 14px;/su,
    );
    expect(timerV2Css).toMatch(/\.scrambleStrip\s*\{[^}]*align-self: stretch;/su);
    expect(timerV2Css).toMatch(/\.scrambleStrip\s*\{[^}]*display: grid;/su);
    expect(timerV2Css).toMatch(
      /\.scrambleStrip\s*\{[^}]*grid-template-rows: minmax\(0, 1fr\) var\(--timer-v2-scramble-toolbar-height\);/su,
    );
    expect(timerV2Css).toMatch(/\.scrambleStrip\s*\{[^}]*align-content: stretch;/su);
    expect(timerV2Css).toMatch(/\.scrambleStrip\s*\{[^}]*align-items: stretch;/su);
    expect(timerV2Css).toMatch(/\.scrambleStrip\s*\{[^}]*max-height: 100%;/su);
    expect(timerV2Css).toMatch(/\.scrambleStrip\s*\{[^}]*min-height: 0;/su);
    expect(timerV2Css).toMatch(
      /\.scrambleStrip\s*\{[^}]*width: min\(\s*1280px,\s*calc\(\s*100vw - var\(--timer-v2-page-inline-padding\) - var\(--timer-v2-page-inline-padding\)\s*\)\s*\);/su,
    );
    expect(timerV2Css).not.toMatch(/\.scrambleStrip\s*\{[^}]*font-size:/su);
    expect(timerV2Css).toMatch(
    ...
    expect(timerV2Css).toMatch(/\.scrambleText\s*\{[^}]*align-items: center;/su);
    expect(timerV2Css).toMatch(/\.scrambleText\s*\{[^}]*align-self: stretch;/su);
    expect(timerV2Css).toMatch(/\.scrambleText\s*\{[^}]*height: 100%;/su);
    expect(timerV2Css).toMatch(/\.scrambleText\s*\{[^}]*max-height: 100%;/su);
    expect(timerV2Css).toMatch(/\.scrambleText\s*\{[^}]*min-height: 0;/su);
    expect(timerV2Css).toMatch(/\.scrambleText\s*\{[^}]*overflow: auto;/su);
    expect(timerV2Css).toMatch(/\.scrambleText\s*\{[^}]*scrollbar-width: none;/su);
    expect(timerV2Css).toMatch(
      /\.scrambleToolbarSlot\s*\{[^}]*min-height: var\(--timer-v2-scramble-toolbar-height\);/su,
    );
    expect(timerV2Css).toMatch(/\.scrambleToolbarPlaceholder\s*\{[^}]*display: inline-flex;/su);
    expect(timerV2Css).toMatch(/\.scrambleToolbarPlaceholderItem\s*\{[^}]*border-radius: 999px;/su);
    expect(timerV2Css).not.toMatch(/@media \(max-width: 980px\) and \(min-height: 1200px\)/u);
    expect(timerV2Css).toMatch(
    ...
    expect(timerV2Css).toMatch(
      /@media \(max-width: 1180px\)\s*\{\s*\.root\s*\{[^}]*--timer-v2-nav-zone-height: calc\(68px \+ env\(safe-area-inset-bottom\)\);/su,
    );
    expect(timerV2Css).toMatch(
      /@media \(max-width: 1180px\)\s*\{\s*\.root\s*\{[^}]*--timer-v2-top-zone-height: clamp\(256px, 30vh, 340px\);/su,
    );
    expect(timerV2Css).toMatch(
      /@media \(max-width: 1180px\)\s*\{\s*\.root\s*\{[^}]*grid-template-areas:\s*'hero'\s*'stage'\s*'bottom'\s*'nav';/su,
    );
    expect(timerV2Css).toMatch(
      /@media \(max-width: 1180px\)\s*\{\s*\.root\s*\{[^}]*grid-template-rows:\s*var\(--timer-v2-top-zone-height\)\s*minmax\(0, 1fr\)\s*var\(--timer-v2-bottom-zone-height\)\s*var\(--timer-v2-nav-zone-height\);/su,
    );
    expect(timerV2Css).toMatch(
    ...
    expect(timerV2Css).toMatch(
      /@media \(max-width: 720px\)\s*\{[^}]*--timer-v2-top-zone-height: 342px;/su,
    );
    expect(timerV2Css).toMatch(
      /@media \(max-width: 720px\)\s*\{[^}]*--timer-v2-page-inline-padding: 16px;/su,
    );
    expect(timerV2Css).toMatch(
      /@media \(max-width: 720px\)\s*\{[^}]*--timer-v2-bottom-zone-height: 126px;/su,
    );
    expect(timerV2Css).toMatch(
      /@media \(max-width: 720px\)\s*\{[^}]*--timer-v2-nav-zone-height: calc\(58px \+ env\(safe-area-inset-bottom\)\);/su,
    );
    expect(timerV2Css).toMatch(
      /@media \(max-width: 720px\)\s*\{[^}]*--timer-v2-scramble-toolbar-height: 30px;/su,
    );
    expect(timerV2Css).toMatch(
    ...
    expect(timerV2Css).not.toMatch(/-webkit-line-clamp:/u);
    expect(timerV2Css).not.toMatch(/display: -webkit-box;/u);
    expect(timerV2Css).toMatch(
      /\.bottomDock\s*\{[^}]*grid-template-columns: minmax\(0, 1fr\) minmax\(0, 3fr\);/su,
    );
    expect(timerV2Css).toMatch(
      /\.bottomDock\s*\{[^}]*padding: 0 var\(--timer-v2-page-inline-padding\) calc\(16px \+ env\(safe-area-inset-bottom\)\);/su,
    );
    expect(timerV2Css).toMatch(/\.bottomDock\s*\{[^}]*height: 100%;/su);
    expect(timerV2Css).toMatch(/\.bottomDock\s*\{[^}]*min-height: 0;/su);
    expect(timerV2Css).toMatch(
    ...
    expect(timerV2Css).toMatch(
      /\.timerSurface\s*\{[^}]*transform: translateY\(var\(--timer-v2-stage-center-offset\)\);/su,
    );
    expect(timerV2Css).toMatch(
      /\.timeFace\s*\{[^}]*--timer-v2-time-wide-size: clamp\(4\.6rem, 15vw, 10\.4rem\);/su,
    );
    expect(timerV2Css).toMatch(
      /\.timeFace\s*\{[^}]*--timer-v2-time-max-size: clamp\(4\.2rem, 16vw, 9\.4rem\);/su,
    );
    expect(timerV2Css).not.toMatch(/--timer-v2-time-size:/u);
    expect(timerV2Css).toMatch(
      /\.timeFace\s*\{[^}]*font-size: var\(--timer-v2-time-wide-size\);/su,
    );
    expect(timerV2Css).not.toMatch(/\.timeFace\[data-time-width='wide'\]/u);
    expect(timerV2Css).toMatch(
      /\.timeFace\[data-time-width='max'\]\s*\{[^}]*font-size: var\(--timer-v2-time-max-size\);/su,
    );
    expect(timerV2Css).toMatch(/\.timeFace\s*\{[^}]*font-feature-settings: 'tnum' 1;/su);
    expect(timerV2Css).toMatch(/\.timeFace\s*\{[^}]*font-variant-numeric: tabular-nums;/su);
    expect(timerV2Css).toMatch(
    ...
    expect(timerV2Css).toMatch(/\.timerFraction\s*\{[^}]*font-size: inherit;/su);
    expect(timerV2Css).toMatch(/\.timerFraction\s*\{[^}]*font-variant-numeric: inherit;/su);
    expect(timerV2Css).toMatch(/\.timerFraction\s*\{[^}]*font-weight: inherit;/su);
    expect(timerV2Css).not.toMatch(/\.timerFraction\s*\{[^}]*font-weight: 380;/su);
    expect(timerV2Css).not.toMatch(/\.timerFraction\s*\{[^}]*font-size: 0\.8em;/su);
    expect(timerV2Css).toMatch(/\.timerFraction\s*\{[^}]*margin-inline-start: 0;/su);
    expect(timerV2Css).toMatch(/\.timerGlyph\s*\{[^}]*--timer-v2-glyph-width: 0\.58em;/su);
    expect(timerV2Css).toMatch(/\.timerGlyph\s*\{[^}]*flex: 0 0 var\(--timer-v2-glyph-width\);/su);
    expect(timerV2Css).toMatch(
      /\.timerGlyph\[data-timer-glyph='separator'\]\s*\{[^}]*--timer-v2-glyph-width: 0\.32em;/su,
    );
    expect(timerV2Css).toMatch(
      /\.summaryMetric\s*\{[^}]*display: flex;[^}]*justify-content: flex-start;/su,
    );
    expect(timerV2Css).not.toMatch(/\.summaryMetric\s*\{[^}]*grid-template-columns:/su);
    expect(timerV2Css).toMatch(/\.summaryLabel\s*\{[^}]*font-size: 0\.86rem;/su);
    expect(timerV2Css).toMatch(/\.summaryValue\s*\{[^}]*font-size: 0\.86rem;/su);
    expect(timerV2Css).toMatch(/\.scramblePreview\s*\{[^}]*align-self: stretch;/su);
    expect(timerV2Css).toMatch(
    ...
    expect(timerV2Css).toMatch(/\.stage\s*\{[^}]*position: relative;/su);
    expect(timerV2Css).toMatch(/\.recentRail\s*\{[^}]*display: none;/su);
    expect(timerV2Css).toMatch(
      /\.recentRail\s*\{[^}]*inset-inline-end: var\(--timer-v2-page-inline-padding\);/su,
    );
    expect(timerV2Css).toMatch(/\.recentRail\s*\{[^}]*max-height: min\(360px, 44vh\);/su);
    expect(timerV2Css).toMatch(
      /@media \(min-width: 1180px\) and \(min-height: 720px\)\s*\{[^}]*\.recentRail\s*\{[^}]*display: grid;/su,
    );
    expect(timerV2Css).toMatch(/\.root\[data-focus-mode='true'\] \.recentRail/su);
    expect(timerV2Css).toMatch(
      /@media \(max-height: 520px\) and \(orientation: landscape\)\s*\{[^}]*--timer-v2-bottom-zone-height: 0px;/su,
    );
    expect(timerV2Css).toMatch(
      /@media \(max-height: 520px\) and \(orientation: landscape\)\s*\{[^}]*--timer-v2-nav-zone-height: calc\(52px \+ env\(safe-area-inset-bottom\)\);/su,
    );
    expect(timerV2Css).toMatch(
      /@media \(max-height: 520px\) and \(orientation: landscape\)\s*\{[^}]*--timer-v2-top-zone-height: clamp\(180px, 48vh, 204px\);/su,
    );
    expect(timerV2Css).toMatch(
      /@media \(max-height: 520px\) and \(orientation: landscape\)\s*\{[^}]*--timer-v2-stage-center-offset: 0px;/su,
    );
    expect(timerV2Css).toMatch(
      /@media \(max-height: 520px\) and \(orientation: landscape\)\s*\{[\s\S]*?\.bottomDock\s*\{[^}]*display: none;/u,
    );
    expect(timerV2Css).toMatch(
      /@media \(max-height: 520px\) and \(orientation: landscape\)\s*\{[\s\S]*?\.timerSurface\s*\{[^}]*--timer-v2-feedback-height: 32px;/u,
    );
    expect(timerV2Css).toMatch(
      /@media \(max-height: 520px\) and \(orientation: landscape\)\s*\{[\s\S]*?\.timerSurface\s*\{[^}]*--timer-v2-feedback-offset: clamp\(2\.35rem, 9\.8vh, 2\.55rem\);/u,
    );
    expect(timerV2Css).toMatch(
      /@media \(max-height: 520px\) and \(orientation: landscape\)\s*\{[\s\S]*?\.timeFace\s*\{[^}]*--timer-v2-time-wide-size: clamp\(3rem, 17vh, 4\.4rem\);/u,
    );
    expect(timerV2Css).toMatch(
      /@media \(max-height: 520px\) and \(orientation: landscape\)\s*\{[\s\S]*?\.timeFace\s*\{[^}]*--timer-v2-time-max-size: clamp\(2\.8rem, 16vh, 4\.1rem\);/u,
    );
    expect(timerV2Css).toMatch(
  ...
  it('starts after Space is released and stops with Enter', () => {
    ...
    const pageRoot = container.querySelector('[aria-label="新版计时器"]');
  ...
  it('shows the newest solve at the bottom in the wide-screen recent solve rail', () => {
    ...
    expect(timerV2Css).not.toMatch(/\.recentRailIndex/u);
    expect(timerV2Css).toMatch(
      /\.recentRailItem:last-child \.recentRailTime\s*\{[^}]*opacity: 1;/su,
    );
    expect(timerV2Css).toMatch(
  ...
  it('uses the max timer width bucket for long clock values', () => {
    ...
    expect(timerV2PageSource).toMatch(/formatElapsedClock/su);
    expect(timerV2PageSource).toMatch(/splitTimerV2ElapsedText/su);
  });
  ...
  it('prevents selecting timer and result action text', () => {
    expect(timerV2Css).toMatch(/\.timeFace\s*\{[^}]*user-select: none;/su);
    expect(timerV2Css).toMatch(/\.resultToolbar\s*\{[^}]*user-select: none;/su);
    expect(timerV2Css).toMatch(/\.resultButton\s*\{[^}]*user-select: none;/su);
  });

  it('keeps result action spacing and hover feedback visually consistent', () => {
    expect(timerV2Css).toMatch(/\.resultToolbar\s*\{[^}]*gap: 8px;/su);
    expect(timerV2Css).toMatch(/\.resultButton\s*\{[^}]*min-width: 52px;/su);
    expect(timerV2Css).toMatch(/\.resultButton\s*\{[^}]*opacity: 0\.62;/su);
    expect(timerV2Css).toMatch(/\.resultButton:hover\s*\{[^}]*opacity: 1;/su);
    expect(timerV2Css).not.toMatch(/\.resultButton:hover\s*\{[^}]*text-decoration: underline;/su);
  });
});
...
// @ts-expect-error The app tsconfig intentionally excludes Node types, while Vitest still runs in Node.
const { readFileSync } = await import('node:fs');
const timerV2Css = readFileSync(
  `${process.cwd()}/src/timer-v2/timer-v2-page.module.css`,
  'utf8',
) as string;
const timerV2PageSource = readFileSync(
  `${process.cwd()}/src/timer-v2/timer-v2-page.tsx`,
  'utf8',
) as string;
const timerV2NavigationSource = readFileSync(
  `${process.cwd()}/src/timer-v2/timer-v2-navigation.tsx`,
  'utf8',
) as string;
